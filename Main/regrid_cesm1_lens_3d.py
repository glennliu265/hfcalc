...
["TAUX","TAUY","PRECC","PRECL","PRECSC","PRECSL"]
#
#
#["HMXL]
#["LANDFRAC","ICEFRAC","SHFLX","LHFLX","FSNS","FLNS"]
new_res = 5 # New resolution (in degrees)
method  = 'bilinear'
idz     = 0 # Level Index (for 3d Variables)
...
for vv in range(nvars):
    ...
    for e in tqdm(range(nens)):
        ...
        if idz is not None: # Works for ocean variables... (took 66.30s)
            dsvar = dsvar.isel(z_t=idz)
        dsvar = dsvar.load()
        print("Loaded variable in %.2fs" % (time.time()-st))
    ...
    savename  = "%s%s_%s_Global_1920to2005.nc" % (outpath,datname,vname)
